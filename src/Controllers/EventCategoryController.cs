...
namespace BachelorTherasoftDotnetApi.src.Controllers
{
    ...
    public class EventCategoryController : ControllerBase
    {
        ...
        {
            var res = await _eventCategoryService.GetByIdAsync(workspaceId, id);
            return Ok(res);
        }
        ...
        public async Task<IActionResult> Create([FromRoute] string workspaceId, [FromBody] CreateEventCategoryRequest request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState.Values.SelectMany(x => x.Errors).Select(y => y.ErrorMessage).ToList());

            var res = await _eventCategoryService.CreateAsync(workspaceId, request);
        ...
        {
            var res = await _eventCategoryService.DeleteAsync(workspaceId, id);
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> Update([FromRoute] string workspaceId, [FromRoute] string id, [FromBody] UpdateEventCategoryRequest request)
        {
            if (!ModelState.IsValid) return BadRequest(ModelState.Values.SelectMany(x => x.Errors).Select(y => y.ErrorMessage).ToList());
            ...
            var res = await _eventCategoryService.UpdateAsync(workspaceId, id, request);
            return Ok(res);
        }
